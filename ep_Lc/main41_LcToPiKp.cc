...
vector<int> relevantPDGs = {211,  321, 2212};
...
int main(int argc, char *argv[])
{
  ...
  while (EventsCount < Total_Count)
      ...
      for (int i=0; i<event.size(); i++)
	{
	  ...
	  if (abs(currentParticle.id()) != 4122) continue;
	  vector<int> daughters = currentParticle.daughterList();
	  if (daughters.size() != 3) continue;
	  vector<int> daughtersID = {abs(pythia.event[daughters[0]].id()), abs(pythia.event[daughters[1]].id()), abs(pythia.event[daughters[2]].id())};
	  if (areVectorsDifferentIgnoreOrder(daughtersID, relevantPDGs)) continue;
	     
	  printf("Mother %s, Daughters: %s %s %s \n",
		 ...
		 pythia.particleData.name(pythia.event[daughters[2]].id()).c_str());
  ...
  cout<<"Total events scanned = "<<EventsCount<<", Lc found = " << Mothercount << endl;
  // Done.
  return 0;
}
